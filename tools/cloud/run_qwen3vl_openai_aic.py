...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a Qwen3-VL OpenAI-compatible vision-language server over "
            "the unlabeled AIC test set."
        )
    )
    ...
    parser.add_argument("--model", default="Qwen3-VL-30B-A3B-Instruct-FP8")
...
def _prompt(query: str) -> str:
    return (
        "You are solving visual grounding. Locate the object described by the "
        "query in the image. Output exactly one bounding box as JSON array "
        "[x1, y1, x2, y2]. Use normalized coordinates from 0 to 1 relative "
        "to the visible RGB image. The benchmark always expects one target. "
        "If the target is hard to see, partially occluded, tiny, or ambiguous, "
        "still output your best-guess bounding box for the most likely target. "
        "Never answer that the object is absent, unknown, impossible, none, or "
        "not found. Do not output explanations or extra text.\n"
        f"Query: {query}"
    )
...
async def _predict_one(
    *,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            row = {
                ...
                "selection_policy": "qwen3vl_openai_single_bbox",
                "selected_index": 0,
                "selected_bbox": bbox,
    ...
    if fallback == "error":
        raise RuntimeError(f"{record.query_id}: Qwen3-VL prediction failed: {error_text}")
    bbox = _fallback_bbox()
    latency_ms = (time.perf_counter() - started) * 1000.0
    row = {
        "query_id": record.query_id,
...
async def _run(args: argparse.Namespace) -> int:
    ...
    fingerprint = {
        "schema": "aic-qwen3-vl-openai-zero-shot-v1",
