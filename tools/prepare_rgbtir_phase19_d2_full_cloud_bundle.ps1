param(
    [string]$OutputZip = "D:\12525\Documents\pytorch\aic_rgbtir_phase19_d2_full_cloud_bundle_20260816.zip"
)

$ErrorActionPreference = "Stop"
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$OutputParent = Split-Path -Parent $OutputZip
$StageRoot = Join-Path $OutputParent "aic_rgbtir_phase19_d2_full_bundle"
$PreparedRoot = Join-Path $RepoRoot "outputs\aic_rgbtir_phase19_d2_v1"
$ReturnedRoot = Join-Path $RepoRoot "outputs\aic_rgbtir_phase19_d2_returned_20260816\extracted\outputs\aic_rgbtir_phase19_d2_probe_v1"

$Required = @(
    (Join-Path $PreparedRoot "manifests\d2_remaining_full_train.jsonl"),
    (Join-Path $PreparedRoot "manifests\d2_semantic_dev.jsonl"),
    (Join-Path $PreparedRoot "manifests\d2_multiquery_dev.jsonl"),
    (Join-Path $PreparedRoot "manifests\official_val.jsonl"),
    (Join-Path $PreparedRoot "assets\teacher_bank\teacher_bank.pt"),
    (Join-Path $PreparedRoot "assets\d1_selected\adapter.pt"),
    (Join-Path $ReturnedRoot "run_summary.json"),
    (Join-Path $RepoRoot "configs\aic_rgbtir_phase19_d2_full.cloud.example.yaml"),
    (Join-Path $RepoRoot "tools\run_rgbtir_phase19_d2_full.py")
)
foreach ($Path in $Required) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Missing Phase 1.9-D2 Full bundle asset: $Path"
    }
}
...
$Directories = @(
    ...
    "assets\manifests", "assets\teacher_bank", "assets\d1_selected",
    "assets\probe_evidence", "reports"
)
...
foreach ($Name in @(
    "d2_remaining_full_train.jsonl",
    "d2_semantic_dev.jsonl",
    "d2_multiquery_dev.jsonl",
    "official_val.jsonl"
)) {
...
Copy-Item -LiteralPath (Join-Path $ReturnedRoot "run_summary.json") -Destination (Join-Path $StageRoot "assets\probe_evidence")

$Expected = @{
    "assets/manifests/d2_remaining_full_train.jsonl" = "01587257901598FB072CD72AFE794C2DCB3A9F0E074750FB4B09484EBB390CA4"
    ...
    "assets/probe_evidence/run_summary.json" = "895C260ADF8C7E26C02F6046AC362C16AD7D33AE1025871C4CBA0453F87C8D8A"
}
foreach ($Relative in $Expected.Keys) {
    ...
    if ($Actual -ne $Expected[$Relative]) {
        throw "Phase 1.9-D2 Full bundle hash mismatch for ${Relative}: $Actual"
    }
}
...
$Manifest = @{
    schema_version = 1
    phase = "1.9-D2-Full"
    scope = "D2_G025_FULL_TRAIN_DUAL_DEV_ONE_SHOT_OFFICIAL"
...
try {
    Get-ChildItem -LiteralPath $StageRoot -Recurse -File | Sort-Object FullName | ForEach-Object {
        ...
        [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
            $Archive,
            $_.FullName,
            "aic_rgbtir_phase19_d2_full_bundle/$Relative",
...
@{
    status = "PHASE_19_D2_FULL_CLOUD_BUNDLE_READY"
    zip = $OutputZip
